...
export default function createFaceFromPoints(context, payload) {
  ...
  withPreservedComponents(context, currentStoryGeometry.id, () => {
    ...
    const affectedGeometry = {
      edges: faceGeometry.edges.concat(currentStoryGeometry.edges.slice(oldEdgeLength)),
      vertices: faceGeometry.vertices.concat(currentStoryGeometry.vertices.slice(oldVertexLength)),
    };

    // save the face and its descendent geometry
    storeFace(faceGeometry, target, context, existingFace);

    // replaceFacePoints may reuse vertices and edges that were stored while overlapping
    // faces were updated. Reconcile the affected geometry after all faces have been stored,
    // so edge splitting never receives references to the discarded ids.
    const matchedGeometry = matchFaceGeometryToExistingGeometry(
      affectedGeometry,
      currentStoryGeometry,
      context.rootState.project.grid.spacing,
    );
    let newEdges = _.uniqBy(matchedGeometry.edges, 'id');
    let newVertices = _.uniqBy(matchedGeometry.vertices, 'id');
    ...
    // Trimming can remove affected geometry that is no longer referenced by a face.
    newEdges = newEdges.filter(edge => geometryHelpers.edgeForId(edge.id, currentStoryGeometry));
    newVertices = newVertices.filter(vertex => geometryHelpers.vertexForId(vertex.id, currentStoryGeometry));
